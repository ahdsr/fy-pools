...
export async function submitRankedFinishPicks({ inviteCode: inviteCodeValue, payload, templateSlug }: { inviteCode: string; payload: RankedFinishPickPayload; templateSlug: string }) {
  ...
  if (error) throw new Error(error.message); const row = Array.isArray(data) ? data[0] : data; if (!row?.entry_id) throw new Error("Picks were not submitted."); revalidatePath(`/pools/${join.pool.slug}`); return { entryId: String(row.entry_id), entryPickId: String(row.entry_pick_id), submittedAt, poolSlug: join.pool.slug };
}
...
export async function updateRankedFinishPoolBasics({ poolId, templateSlug, basics }: { poolId: string; templateSlug: string; basics: Pick<RankedFinishSettings["basics"], "poolName" | "commissionerName" | "description"> }) {
  const user = await ensureUser(); const admin = createSupabaseAdminClient();
  ...
  const settings = rankedSettings(pool.settings, templateSlug);
  if (!settings) throw new Error("Ranked-finish settings were not found.");
  ...
  const { error: updateError } = await admin.from("pools").update({ name: poolName, settings: { rankedFinish: nextSettings }, updated_at: new Date().toISOString() }).eq("id", poolId);
  if (updateError) throw new Error(updateError.message);
  await audit(admin, poolId, user.id, "ranked_finish.pool_details_updated", `Updated pool details for ${poolName}.`, {});
