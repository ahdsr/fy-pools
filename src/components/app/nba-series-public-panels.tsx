...
export function NbaSeriesLeaderboard({ pool, preview = false }: { pool: NbaPublicPool; preview?: boolean }) {
  ...
  const hasMoreRows = (hasStandings ? pool.latestStandings.length : pool.entries.length) > (hasStandings ? standings.length : entries.length);
  return <LedgerPanel title="Leaderboard" description="Standings refresh whenever the commissioner simulates a completed series."><LedgerRows>{hasStandings ? standings.map((row) => <LedgerRow key={row.entryId} className="flex items-center justify-between gap-4"><div><p className="font-semibold text-brand-ink">{row.rank}. {row.entryName}</p><p className="text-sm text-muted-foreground">Max {row.maxPoints} points</p></div><p className="text-xl font-bold text-brand-ink">{row.total}</p></LedgerRow>) : entries.map((entry) => <LedgerRow key={entry.entryId}><p className="font-semibold text-brand-ink">{entry.entryName}</p><p className="text-sm text-muted-foreground">Submitted bracket awaiting a result.</p></LedgerRow>)}{!pool.entries.length ? <LedgerRow><p className="text-sm text-muted-foreground">No submitted brackets yet.</p></LedgerRow> : null}</LedgerRows>{preview && hasMoreRows ? <div className="border-t px-5 py-4"><Link href={`/pools/${pool.poolSlug}/leaderboard`} className="text-sm font-semibold text-brand-ink hover:text-brand-hot">View full standings</Link></div> : null}</LedgerPanel>;
}
