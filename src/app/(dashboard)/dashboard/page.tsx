...
import { ArchivePoolButton } from "./pools/archive-pool-button";
...
function PoolSummaryCard({ pool }: { pool: CommissionerPoolSummary }) {
  ...
  const archived = pool.status.toLowerCase() === "archived";

  return <article className="flex min-h-[16rem] flex-col justify-between gap-5 px-5 py-6"><div className="space-y-4"><div className="flex items-center gap-2"><h2 className="min-w-0 text-xl font-bold tracking-normal text-brand-ink">{pool.poolName}</h2><button type="button" aria-label={`About ${pool.poolName}`} aria-describedby={infoId} className="group relative inline-flex size-6 shrink-0 items-center justify-center rounded-full border border-border bg-surface-paper text-muted-foreground transition hover:border-primary/35 hover:text-brand-ink focus-visible:border-ring focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/25"><Info className="size-3.5" aria-hidden="true" /><span id={infoId} role="tooltip" className="pointer-events-none absolute left-0 top-[calc(100%+0.5rem)] z-30 w-72 max-w-[calc(100vw-3rem)] rounded-md border bg-popover px-3 py-2 text-left font-sans text-xs font-normal leading-5 tracking-normal text-popover-foreground opacity-0 shadow-lg transition group-hover:opacity-100 group-focus-visible:opacity-100">{poolDetails}</span></button><div className="ml-auto flex shrink-0 items-center gap-1"><Button asChild variant="outline" size="icon-sm"><Link href={`/dashboard/pools/${pool.poolId}/edit`} aria-label={`Edit ${pool.poolName}`} title={`Edit ${pool.poolName}`}><Edit /></Link></Button><ArchivePoolButton poolId={pool.poolId} poolName={pool.poolName} archived={archived} /><DeletePoolButton poolId={pool.poolId} poolName={pool.poolName} entryCount={pool.entryCounts.total} inviteCount={pool.inviteCounts.total} iconOnly /></div></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 xl:divide-x"><PoolMetric icon={Users} label="Direct invites">{pool.inviteCounts.accepted} of {pool.inviteCounts.total} claimed</PoolMetric><PoolMetric icon={Users} label="Shared link">{pool.inviteCounts.shareLinkJoins} joined</PoolMetric><PoolMetric icon={ListChecks} label="Entries">{submittedEntries} submitted</PoolMetric><PoolMetric icon={CalendarClock} label="Deadline">{formatDateTime(pool.pickDeadline)}</PoolMetric></div></div><div className="flex flex-wrap gap-2">{archived ? <p className="text-sm font-medium text-muted-foreground">Archived — participant links are unavailable.</p> : <><Button asChild variant="primaryGreen"><Link href={`/pools/${pool.poolSlug}/leaderboard`}>Leaderboard <Trophy /></Link></Button>{isActivePool(pool) && pool.makePicksHref ? <Button asChild variant="outline"><Link href={pool.makePicksHref}>Make picks <ListChecks /></Link></Button> : null}{isActivePool(pool) ? <ShareLinkButton href={pool.shareInviteHref} /> : null}</>}</div></article>;
}
