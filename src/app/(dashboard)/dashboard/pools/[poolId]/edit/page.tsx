...
import { getCommissionerF1Pool, getCommissionerGolfPool } from "@/lib/ranked-finish/persistence";
...
import { GolfCommissioner } from "./golf-commissioner";
...
async function EditPoolWizard({ params }: EditPoolPageProps) {
  ...
  if (!pool) {
    ...
    const golfPool = await getCommissionerGolfPool(poolId);
    if (golfPool) return <PageShell eyebrow="PGA Tour" title={golfPool.poolName} description="Record final finishing positions, then watch standings update." showHeader={false}><GolfCommissioner poolId={golfPool.poolId} poolName={golfPool.poolName} settings={golfPool.settings} /></PageShell>;
  }
