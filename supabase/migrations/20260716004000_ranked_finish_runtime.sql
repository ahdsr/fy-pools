-- Ranked-finish lock dispatch and atomic F1 (and future placement-pool) submissions.

create or replace function public.ranked_finish_effective_pick_lock_at(p_settings jsonb)
returns timestamptz
language plpgsql
set search_path = pg_catalog
as $$
declare
  v_timezone text := coalesce(nullif(trim(p_settings #>> '{rankedFinish,basics,timezone}'), ''), 'UTC');
  v_value text := p_settings #>> '{rankedFinish,basics,picksLockAt}';
begin
...
begin
  ...
  elsif v_settings ? 'rankedFinish' then
    v_pick_lock_at := public.ranked_finish_effective_pick_lock_at(v_settings);
  else
...
begin
  ...
  v_lock_at := public.ranked_finish_effective_pick_lock_at(v_settings);
...
revoke all on function public.ranked_finish_effective_pick_lock_at(jsonb) from public;
...
grant execute on function public.ranked_finish_effective_pick_lock_at(jsonb) to service_role;
