-- Fix the remaining PL/pgSQL ambiguity in the Round of 16 submission RPC.
-- The output column `entry_pick_id` can shadow the entry_pick_items column name
-- inside an `on conflict (entry_pick_id, template_pick_field_id)` target.
...
begin
  ...
  on conflict on constraint entry_pick_items_entry_pick_id_template_pick_field_id_key
  do update
  set
